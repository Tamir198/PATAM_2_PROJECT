...
public class FlightList implements Initializable {
    ...
    public void deleteFlight() {
        ...
        if (selectedItem == null) {
            // display an error message for not selecting a row to delete
            status.setText("Please select a row and perform this action again");
        } else {
            // create the ObjectId from id string
            ObjectId flight_id = new ObjectId(selectedItem.getFlight_id());

            // here we are using the ObjectId as primary key to find each document to delete from the database
